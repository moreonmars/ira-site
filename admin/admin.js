(() => {
  ...
  const renderEditor = () => {
    ...
    editor.querySelector('#cover-upload').addEventListener('change', async event => { const [file] = event.target.files; if (!file) return; try { const url = await upload(file); editor.querySelector('#cover-field').value = url; editor.querySelector('.cover-preview img').src = url; toast('Обкладинку завантажено'); } catch (error) { toast(error.message); } });
  ...
  document.querySelector('#new-work').addEventListener('click', () => { const id = `new-work-${Date.now()}`; works.unshift({ id, status: 'draft', year: new Date().getFullYear().toString(), location: { uk: 'Нове місце', en: 'New location' }, title: { uk: 'НОВА РОБОТА', en: 'NEW WORK' }, description: { uk: '', en: '' }, photographer: { uk: '', en: '' }, cover: '../assets/hero-green.webp', gallery: '../assets/hero-green.webp' }); selectedId = id; renderList(); renderEditor(); });
